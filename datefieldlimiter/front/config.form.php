...
global $DB; // Already here, so add log after this block
if (isset($_SESSION['glpicsrftokens'])) {
    error_log("DateFieldLimiter CSRF DEBUG - Initial state of \$_SESSION['glpicsrftokens'] on page load: " . print_r($_SESSION['glpicsrftokens'], true));
} else {
    error_log("DateFieldLimiter CSRF DEBUG - Initial state of \$_SESSION['glpicsrftokens'] on page load: Not set");
}
...
if (isset($_POST['save_config'])) {
    if (isset($_SESSION['glpicsrftokens'])) {
        error_log("DateFieldLimiter CSRF DEBUG - Validating POST. Session tokens BEFORE check: " . print_r($_SESSION['glpicsrftokens'], true));
    } else {
        error_log("DateFieldLimiter CSRF DEBUG - Validating POST. Session tokens BEFORE check: Not set");
    }
    error_log("DateFieldLimiter CSRF DEBUG - Token from POST for validation: " . (isset($_POST['_glpi_csrf_token']) ? $_POST['_glpi_csrf_token'] : "NOT SET"));
